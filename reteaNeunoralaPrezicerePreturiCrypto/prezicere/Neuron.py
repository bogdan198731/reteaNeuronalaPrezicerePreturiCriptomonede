...
from prezicere.predictiiOK import PredictiiOK
...
class NeuronC():
    ...
    def __init__(self, x, cripto):
        ...
        while i < self.X:
            ...
            gf = []
            while t < 6:
                gc.append(random.random() * 200 - 100)
    ...
    def predictieF(self, y, greutateCM):
        t = numpy.array(self.criopto.returnMatCripto(y, self.X)).round(5)
        W1 = 3.1  # Open
        W2 = 2.9  # High
        W3 = 2.9  # Low
        ...
        return float(numpy.linalg.det(numpy.dot(t.T, greutateCM)))
    ...
    def verPreictie(self, y):
        ...
        if (pred > pretulUrmator + pretulUrmator / 100 * procenrRecalculare):
            ...
            self.test = True
        elif((pred < pretulUrmator + pretulUrmator / 100 * preocentScriere) and pred > (pretulUrmator - pretulUrmator / 100 * preocentScriere) and self.test):
            ...
            for el in self.greutateCM:
                ...
                listaF.append(lista)
                lista = []
            # self.greutatiOK.append(lista)
            self.greutatiOK.append(listaF)
            self.nrGreutatiOk = self.nrGreutatiOk + 1
            self.test = False
            #self.save(self.nrScriere)
            # self.nrScriere = self.nrScriere + 1
        if y % 2000 == 0:
            print(" Predictie = {:0.2f} , pret real = {:0.2f}".format(pred, pretulUrmator))
            # print(self.greutateCM)
